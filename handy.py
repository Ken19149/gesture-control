...
JOY_DEADZONE = 0.05  # How far you must move your hand to trigger a key (5% of screen)
KEY_CODES = {'W': 17, 'A': 30, 'S': 31, 'D': 32} # Linux evdev keycodes
key_states = {'W': False, 'A': False, 'S': False, 'D': False}
joy_center = None

def update_key(key, press):
    """Only sends a command to ydotool if the key state actually changes."""
...
# 1. Model setup (Now tracking TWO hands)
...
# 2. State Tracking Variables (Right Hand)
...
while cap.isOpened():
    ...
    # Draw a faint split-screen line down the middle
    h, w, c = img.shape
    cv2.line(img, (w//2, 0), (w//2, h), (50, 50, 50), 2)
    
    if detection_result.hand_landmarks:
        for hand_landmarks in detection_result.hand_landmarks:
            # Determine if this hand is on the Left or Right side of the camera
            ...
            # ==========================================
            # LEFT HAND: THE VIRTUAL WASD JOYSTICK
            # ==========================================
            if is_left_hand:
                # If hand is completely open (all 4 fingers up)
                if index_up and middle_up and ring_up and pinky_up:
                    palm_x, palm_y = hand_landmarks[9].x, hand_landmarks[9].y
                    
                    # Set the anchor point the moment the hand opens
                    if joy_center is None:
                        joy_center = (palm_x, palm_y)
                    
                    # Draw the Deadzone Anchor on screen
                    ...
                    # Calculate if the palm has left the deadzone
                    ...
                    # If you curl your fingers or drop your hand, release all keys instantly
            ...
            # ==========================================
                ...
                # --- GESTURE 4: CLOSE APP (3-Finger Swipe Down) ---
                elif index_up and middle_up and ring_up and pinky_down:
                    pointer_prev_x = None 
                    fist_start_time = 0 
                    trail.clear()
                    
                    # Draw red targeting dots on the three fingers
                    ...
                    # Track the average height of the three fingers
                    avg_y = (index_tip.y + middle_tip.y + ring_tip.y) / 3
                    
                    # Cleaned up the cooldown check here
                    if 'close_prev_y' in locals() and (current_time - last_close_time > COOLDOWN):
                        delta_y = avg_y - close_prev_y
                        
                        # Positive Y means moving DOWN the screen
                        if delta_y > SWIPE_Y_THRESHOLD:
                            print("APP KILLED!")
